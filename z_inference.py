...
def select_file_from_directory(
    path, extensions, message, required_suffix=None, excluded_suffix=None
):
    ...
    files = sorted(
        (
            ...
            and (required_suffix is None or name.lower().endswith(required_suffix))
            and (excluded_suffix is None or not name.lower().endswith(excluded_suffix))
...
def load_models_api(args, device=torch.device("cuda")):
    ...
    if lora_path is not None:
        ...
        if detect_checkpoint_type(lora_checkpoint) != "lora":
...
def run_inference(args, model_bundle, device=torch.device("cuda")):
    ...
    # use_style_residual
    use_style_residual = config["model_params"]["length_regulator"].get(
        "use_style_residual", False
    )
    ...
    prompt_condition, _, codes, commitment_loss, codebook_loss, style_prompt = (
...
if __name__ == "__main__":
    ...
    if args.accompany:
        ...
        accompany_dir = os.path.join(output_dir, "accompany")
        os.makedirs(accompany_dir, exist_ok=True)
        op = os.path.join(accompany_dir, output_name)
