...
class FT_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(speech) < self.sr * duration_setting["min"]:
            print(f"Audio {wav_path} is too short, skipping")
            return self.__getitem__(random.randint(0, len(self)))
        if len(speech) > self.sr * duration_setting["max"]:
            orig_len = len(speech) / self.sr
            max_samples = int(self.sr * duration_setting["max"])
            speech = speech[:max_samples]
            print(f"Audio {wav_path} too long ({orig_len:.1f}s), truncated to first 30s")
        if orig_sr != self.sr:
